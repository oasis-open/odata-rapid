...
import { AndExpressionContext, BasicExpressionContext, CompExpressionContext,
    ...
    SelectOptionContext, 
    SkipOptionContext, 
    TopOptionContext} from './parsers/ODataUriQueryParser';
import { createPrimitiveType, createUnknownType, findProperty, findStructuredType, ISchema, ISchemaType, isCollection, isPrimitiveType, PrimitiveType } from './json-model';
...
export interface ISemanticQueryOptionsParser {
    (syntaxTree: QueryOptionsContext, schema: ISchema, rootType: ISchemaType): ISemanticParseResult<QueryOptionsNode>;
}

export function parseQueryOptionsSemantics(syntaxTree: QueryOptionsContext, schema: ISchema, rootType: ISchemaType): ISemanticParseResult<QueryOptionsNode> {
...
export class BinaryOperatorNode extends SemanticNode {
    ...
    set right(node: SemanticNode) {
        ...
        if (this.children.length === 1) {
            ...
            throw new Error("Adding right child of a binary operation without a left child");
        }
    }
...
export function createBinaryOperator(ctx: ParserRuleContext, type: ISchemaType, operator: string): BinaryOperatorNode {
    const ctor = operator === 'or' ? OrNode
        : operator === 'and' ? AndNode
        : operator === 'eq' ? EqNode
        : operator === 'neq' ? NeqNode
        : operator === 'gt' ? GtNode
        : operator === 'gte' ? GteNode
        : operator === 'lt' ? LtNode
        : operator === 'lte' ? LteNode
...
export class QueryOptionsVisitor extends AbstractParseTreeVisitor<SemanticNode> implements ODataUriQueryVisitor<SemanticNode> {
    schema: ISchema;
    ...
    visitFilterOption(ctx: FilterOptionContext) {
        ...
        const expression = ctx.childCount > 0 &&  ctx.tryGetRuleContext(0, ExpressionContext);
        if (expression) {
            node.expression = expression.accept<SemanticNode>(this);

            if (node.expression && node.expression.schemaType.$Type != PrimitiveType.Boolean) {
    ...
    visitSelectNodeFields(ctx: SelectFieldListContext, selectNode: SelectNode): SemanticNode {
        ...
        if (typeDef) {
            ...
            if (!property) {
                this.addError(
                    `Property '${propName}' does not exist on type '${instanceType.$Type}'`, field);
            }
    ...
    visitExpandNodeFields(ctx: ExpandFieldListContext, expandNode: ExpandNode): SemanticNode {
        ...
        if (typeDef) {
            ...
            if (property?.$Kind !== 'NavigationProperty') {
                this.addError(
                    `Property '${propName}' is not a navigation property`, field);
            }
    ...
    visitOrderField(ctx: OrderFieldContext) {
        ...
        if (typeDef) {
            ...
            if (!property) {
                this.addError(
                    `Property '${propName}' does not exist on type '${instanceType.$Type}'`, ctx);
            }

            if (!isPrimitiveType(property)) {
                this.addError(
                    `Cannot order by a non-primitive type`, ctx);
            }
    ...
    visitBasicExpression(ctx: BasicExpressionContext) {
        ...
        if (identifier) {
            ...
            if (isPrimitiveType(instanceType)) {
                this.addError(`Cannot access property '${propName}' of type '${instanceType.$Type}'`, ctx);
            }
            ...
            if (typeDef) {
                ...
                if (!property) {
                    this.addError(
                        `Property '${propName}' does not exist on type '${instanceType.$Type}'`, identifier);
                }
    ...
    visitBinaryExpression(ctx: ParserRuleContext, type: ISchemaType): SemanticNode {
