...
import { BasicExpressionNode, ExpandNode, ISyntaxToSemanticMap, OrderByNode, PropertyAccessNode, QueryOptionsNode, SelectNode } from "./semantic-model"
import { isPrimitiveType, findStructuredType, ISchema, getAllPropertiesNames, getAllProperties } from "./json-model";
...
export class AutoComplete {
    ...
    constructor(parser: QueryParser, syntaxToSemanticMap: ISyntaxToSemanticMap, schema: ISchema) {
    ...
    private identifierSuggestions(ruleKey: number, rule: CandidateRule): string[] {
    ...
    private selectFieldSuggestions(ruleKey: number, rule: CandidateRule): string[] {
        const node = this.map.getByRuleAndToken(ruleKey, rule.startTokenIndex);
    ...
    private orderBySuggestions(ruleKey: number, rule: CandidateRule): string[] {
        ...
        if (node instanceof OrderByNode) {
            ...
            if (typeDef) {
                ...
                return properties.reduce<string[]>((list, item) => [...list, item, `${item} desc`], []);
    ...
    private basicExpressionSuggestions(ruleKey: number, rule: CandidateRule): string[] {
        const node = this.map.getByRuleAndToken(ruleKey, rule.startTokenIndex);
        if (!node) return [];
