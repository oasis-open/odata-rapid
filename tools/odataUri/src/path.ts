...
export class PathParser {
  ...
  parseSegment(raw: IRawSegment) {
    ...
    if (text === "$count") {
      ...
      this.segments.push(
        new CountSegment(createPrimitiveType(PrimitiveType.Int32), raw)
      );
      ...
      prev instanceof KeySegment ||
      prev instanceof SingletonSegment ||
      PropertySegment ||
      ...
      if (!propType) {
        this.addError(
          `The property '${text}' is not defined on type '${prev.schemaType.$Type}'`,
          raw
        );
  ...
  parseFirstSegment(raw: IRawSegment) {
    ...
    if (!navSource) {
      this.addError(
        `'${text}' is not a navigation source in the Service container`,
        raw
      );
